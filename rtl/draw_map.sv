module draw_map (
    input  logic        clk,
    input  logic        rst_n,
    output logic [11:0] map_addr,      // 12 bitów dla 4096 kafelków
    vga_if.out          out,
    vga_if.in           in,
...
always_comb begin
    ...
    // Granice mapy dla 64 kafelków (2048 pikseli)
    is_inside_map_stage1 = (map_pixel_x >= 16'sd0) && (map_pixel_x < 16'sd2048) && 
                           (map_pixel_y >= 16'sd0) && (map_pixel_y < 16'sd2048);
                           
    // Pobieramy bity [10:5] z pikseli, co daje liczby 0-63 (6 bitów + 6 bitów = 12 bitów adresu)
...
always_comb begin
    ...
    // Sprawdzanie granic dla 2048px na 3 etapie potoku
    is_inside_map_stage3 = (map_pixel_x_d1 >= 16'sd0) && (map_pixel_x_d1 < 16'sd2048) && 
                           (map_pixel_y_d1 >= 16'sd0) && (map_pixel_y_d1 < 16'sd2048);
